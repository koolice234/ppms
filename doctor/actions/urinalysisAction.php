...
if(isset($_POST['uriADD'])){
	...
	if($checkrows>0) {
		echo "<script>alert('Patient Already Done with this Laboratory.');window.location.href='../PatientRecord-Checkup.php';</script>";
			...
			$stmt1 = $connect->prepare("INSERT INTO `urinalysis` (sched_ID,patient_ID,uri_date,uri_color,uri_transparency,uri_pH,uri_specificGravity,uri_sugar,uri_protein,uri_MICtransparency,uri_MICpH,uri_MICspecificGravity) VALUES ('$schedID','$u_id','$date','$ucolor','$utransparency','$uph','$usg','$usugar','$uprotien' ,'$utransparency1','$uph1','$usg1')");
		$stmt2 = $connect->prepare("INSERT INTO transaction(trans_type,trans_date,trans_patientid) VALUES ('Urinalysis','$date','$u_id')");
		$stmt2->execute();
		$stmt1->execute();
